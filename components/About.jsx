import { assets, infoList, toolsData } from '@/assets/assets'; // Ensure toolsData is imported
...
const About = ({ isDarkMode }) => {
  return (
    <motion.div 
      id="about" 
      className="w-full px-4 sm:px-[10%] py-10 scroll-mt-20"
      initial={{ opacity: 0 }} 
    ...
    >
      ...
      <motion.div
        ...
        className="flex flex-col lg:flex-row items-center gap-10 sm:gap-20 my-20"
      >
        {/* Profile Image */}
        <motion.div
        ...
        >
          ...
          >
            {infoList.map(({ icon, iconDark, title, description }, index) => (
              ...
              >
                <Image 
                  src={isDarkMode ? iconDark : icon || '/path/to/fallback-icon.png'} 
                  alt={title} 
                  className="w-7 mt-3"
                  width={28} 
                  height={28}
                />
                <h3 className="my-4 font-semibold prose prose-h1:text-gray-900 dark:text-white">{title}</h3>
